...
void Adafruit_Thermal::writechinese(String hex){
  char str[181];
  char ch;
  int i = 0;
  while (hex[i]){
...
void Adafruit_Thermal::begin(uint8_t heatTime) {
  ...
  writeBytes(11, heatTime, 50); // Heating dots, heat time, heat interval
...
void Adafruit_Thermal::ChineseOn(){
  writeBytes(ASCII_FS, '&');
}

void Adafruit_Thermal::ChineseOff(){
  writeBytes(ASCII_FS, '.');
}
